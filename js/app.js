...
// ==========================================
// INTEGRAÇÃO AGENDA ZIMBRA (EVENTOS DE HOJE)
// ==========================================
...
async function carregarEventosDeHoje() {
  ...
  try {
    ...
    const dadosIcs = await resposta.text();
    ...
    const linhas = dadosIcs.split('\n');
    let eventosHoje = [];
    let eventoAtual = {};
    let lendoEvento = false;
    ...
    for (let i = 0; i < linhas.length; i++) {
      ...
      if (linha === "BEGIN:VEVENT") {
        ...
        lendoEvento = false;
        if (eventoAtual.data && eventoAtual.data.startsWith(dataHojeFormatada)) {
          eventosHoje.push(eventoAtual);
        }
      } else if (lendoEvento) {
        if (linha.startsWith("SUMMARY:")) {
          eventoAtual.titulo = linha.replace("SUMMARY:", "");
        } else if (linha.startsWith("DTSTART")) {
          eventoAtual.data = linha.split(":")[1]; 
        }
      }
    }

    mostrarEventosNoHTML(eventosHoje);

  } catch (erro) {
    console.error("Erro ao carregar a agenda:", erro);
    const listaHtml = document.getElementById("lista-eventos");
    if (listaHtml) {
        listaHtml.innerHTML = "<li>Erro ao carregar eventos.</li>";
    }
  }
}

function mostrarEventosNoHTML(eventos) {
  const listaHtml = document.getElementById("lista-eventos");
  
  // Verificação de segurança caso o elemento HTML não exista na página
  if (!listaHtml) return; 

  listaHtml.innerHTML = ""; 

  if (eventos.length === 0) {
    listaHtml.innerHTML = "<li>Nenhum evento programado para hoje.</li>";
    return;
  }
...
// Isso garante que os eventos só serão carregados quando o site terminar de abrir
document.addEventListener("DOMContentLoaded", () => {
    carregarEventosDeHoje();
});
